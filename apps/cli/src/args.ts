...
const HELP_EXAMPLES = `
Examples:
  omh --profile web                          boot the web profile (same as: omh web)
  omh --profile rescue --from-default-profile web
                                             create rescue from the shipped web template, then boot it
  omh --profile headless "run the tests"     answer one task, print the result, and exit
  omh --profile tui --patch ./extra.yml      boot a custom profile with one extra overlay
  omh --profile tui --resume <session>       arguments after the launcher flags reach the app
  omh --profile web --help                   the web app's own flags and help
...
export function parseDshArgs(argv: readonly string[], version: string): DshInvocation {
  ...
  program
    ...
    .description('omh: boot an Omchor Harness profile — an ordered stack of plugin-bundle patch layers under your own overrides.')
    ...
    .argument('[args...]', 'arguments for the booted profile\'s app (see: omh --profile <name> --help)')
