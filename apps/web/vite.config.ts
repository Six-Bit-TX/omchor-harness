...
const STANDALONE_ERROR = 'apps/web is not a standalone application: bare Vite cannot inject window.__DSH_BOOT__. '
  + 'From a repository checkout, run `pnpm omh web`; an installed package uses `omh web`. '
  + 'For client-plugin HMR, run `pnpm omh web` together with `pnpm run dev:web`.'
const DEFAULT_CLIENT_TITLE = 'Omchor Local Build'
...
function clientDocumentTitle(): Plugin {
  ...
  return {
    ...
    transformIndexHtml(html) {
      return html.replace('<title>Omchor Local Build</title>', `<title>${title}</title>`)
    },
  }
}
