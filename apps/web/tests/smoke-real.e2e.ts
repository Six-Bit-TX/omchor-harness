// Real-host smoke: spawn `omh web` with a real key, walk the full flow
...
function authenticatedWeb(launchUrl: string): Promise<{ origin: string; cookie: string }> {
  ...
  const exchange = (async () => {
    ...
    if (response.status !== 303 || setCookie === null) {
      throw new Error(`omh web authentication returned HTTP ${String(response.status)}`)
    }
    return {
...
function waitForReadyLine(child: ChildProcess): Promise<string> {
  return new Promise((resolveReady, reject) => {
    let out = ''
    const timer = setTimeout(() => { reject(new Error(`omh web not ready in 90s; output:\n${out}`)) }, 90_000)
    const onData = (chunk: Buffer): void => {
      out += chunk.toString()
      const match = /omh web: (http:\/\/[^\s]+)/.exec(out)
    ...
    child.once('exit', (code) => {
      clearTimeout(timer)
      reject(new Error(`omh web exited early (code ${code}); output:\n${out}`))
    })
  })
}
...
// Readiness gate: `omh web` serves every production manifest plugin; until every UI
