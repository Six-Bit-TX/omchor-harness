...
/** Model-visible orientation and acceptance boundary for sessions created through `omh web`. */
function webSurfacePrompt(webUrl: string): string {
  ...
  return `You are interacting with the user through the DeepSeek Harness Web GUI at ${webUrl}. `
    ...
    + 'The apps/web Vite entry builds the shell but is not a standalone application because only omh web injects window.__DSH_BOOT__. '
  ...
  if (config.printUrl || handoffBrowser) {
    ctx.inject(['connection'], (connectionCtx) => {
      ...
      const announceReady = (): void => {
        ...
        if (config.printUrl) {
          console.log(`omh web: ${authenticatedUrl}${lanUrl === undefined ? '' : ` (LAN: ${lanUrl})`}`)
        }
        if (handoffBrowser) {
          console.log('omh web: opening the default browser; pass --no-open to disable')
          void internals.openBrowser(authenticatedUrl).catch((error: unknown) => {
            const reason = error instanceof Error ? error.message : String(error)
            console.error(`web-app: could not open the default browser because ${reason}; use the omh web URL printed at startup`)
          })
        }
      }
      ...
      if (settled === undefined) announceReady()
        ...
        void settled.then(async () => {
          await auditStartupEntries(connectionCtx.root, 'omh web', () => {})
